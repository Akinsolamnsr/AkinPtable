...
export default function NavWrapNew({children}) {
    ...
    <div className={`flex-col flex `} >
      ...
      <div   className={`h-12   w-full flex shadow-lg justify-evenly`}>
        ...
        <span className='hover:text-cyan-500 flex items-center'>
           {size.width<550?(<><Link href="/"><a><Image src={homePic} alt="pics" width="25" heigth="25" /></a></Link></>):(<><Image src={homePic} alt="pics" width="25" heigth="25"  /><span className={` h-full pl-[1.5rem] pr-[1.2rem] -ml-[1.5rem] flex items-center   ${router.pathname == "/home" ? "border-b-4 border-cyan-700 bg-gray-100" : ""}`}><Link href="/"><a> Home</a></Link></span></>)}
           </span>
          <span className='hover:text-cyan-500 flex items-center'>
      ...
      <div className={`h-screen w-[90%] ml-[5%] order-1 md:order-2 flex  ${size.width< 1100?"flex-col":`${Wdth?"flex-col":"flex-row"}`}`}  >
