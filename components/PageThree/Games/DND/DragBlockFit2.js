...
export default function DragDnd2(prop){ 
  ...
  const Block=data[3].split(',')[0]  
    return(   
        ...
        `} ref={drag}>
         {state.flip?<span className='text-center ml-1 mt-2 font-bold'>{data[0]}</span>:<> <div className={`text-center ${MaxSize?"text-[0.5rem] ":"text-[1rem] "} `}>{data[1]}</div><div className={`${MaxSize?"text-[0.5rem] -ml-8":"text-[1rem]"} text-center w-12 -ml-`}>{data[2]}</div></>}
        </div>
        )
}
