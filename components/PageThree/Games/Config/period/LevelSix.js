...
export default function Level6({count}){
  ...
  useMemo(()=>{
    if(val==List[0] && val2==List[1] && val3==List[2] && val4==List[3] && val5==List[4] && val6==List[5]){
    ...
    return(
      ...
      <div className={` flex ${MaxSize?"":"flex-col"} `}>  
      <div className="">
      <input value={val6}  onChange={(e)=>setVal6(e.target.value)} className={`${MaxSize?"w-[50px] h-[50px] text-[1.5rem] text-center border-4 border-pink-400 shadow-xl bg-gray-200":"w-8"} ml-1`} />
      </div>
      <span className="w-8">
